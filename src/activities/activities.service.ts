...
export class ActivitiesService {
  ...
  async create(data: CreateActivityInput, currentUserId: string): Promise<Activity> {
    const canCreate = await this.rbacService.hasPermission(
      currentUserId,
      { resource: ResourceType.ACTIVITY, action: ActionType.CREATE }
    );
    if (!canCreate) {
      throw new ForbiddenException('Insufficient permissions to create activity');
    }
  ...
  async findAll(currentUserId: string, take?: number, skip?: number): Promise<Activity[]> {
    const hasPermission = await this.rbacService.hasPermission(
      currentUserId,
      { resource: ResourceType.ACTIVITY, action: ActionType.READ }
    );
    if (!hasPermission) {
      throw new ForbiddenException('Insufficient permissions to access activities');
    }

    const filters = await this.rbacService.getPermissionFilters(currentUserId, ResourceType.ACTIVITY);
  ...
  async findOne(id: string, currentUserId: string): Promise<Activity> {
    ...
    if (!canRead) {
      throw new ForbiddenException('Insufficient permissions to view this activity');
    }

    return activity;
  }

  async update(id: string, data: UpdateActivityInput, currentUserId: string): Promise<Activity> {
    ...
    if (!canUpdate) {
      throw new ForbiddenException('Insufficient permissions to update this activity');
    }
  ...
  async remove(id: string, currentUserId: string): Promise<Activity> {
    ...
    if (!canDelete) {
      throw new ForbiddenException('Insufficient permissions to delete this activity');
    }

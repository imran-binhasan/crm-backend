...
export class ClientsService {
  ...
  async create(data: CreateClientInput, currentUserId: string): Promise<Client> {
    const canCreate = await this.rbacService.hasPermission(
      currentUserId,
      { resource: ResourceType.CLIENT, action: ActionType.CREATE }
    );
    if (!canCreate) {
  ...
  async findAll(currentUserId: string, take?: number, skip?: number): Promise<Client[]> {
    const hasPermission = await this.rbacService.hasPermission(
      currentUserId,
      { resource: ResourceType.CLIENT, action: ActionType.READ }
    );
    if (!hasPermission) {
      throw new ForbiddenException('Insufficient permissions to access clients');
    }

    const filters = await this.rbacService.getPermissionFilters(currentUserId, ResourceType.CLIENT);
  ...
  async findOne(id: string, currentUserId: string): Promise<Client> {
    ...
    if (!canRead) {
      throw new ForbiddenException('Insufficient permissions to view this client');
    }

    return client;
  }

  async update(id: string, data: UpdateClientInput, currentUserId: string): Promise<Client> {
    ...
    if (!canUpdate) {
      throw new ForbiddenException('Insufficient permissions to update this client');
    }
  ...
  async remove(id: string, currentUserId: string): Promise<Client> {
    ...
    if (!canDelete) {
      throw new ForbiddenException('Insufficient permissions to delete this client');
    }
    ...
    if (projectCount > 0) {
      throw new BadRequestException('Cannot delete client with associated projects. Please delete projects first.');
    }
